...
func NewAccountAPI(ctx context.Context, opt *Options) (api account.AccountAPIServer, err error) {
	...
	case opt.AppName == "":
		err = errors.New("missing app name")
	case opt.EmailDisplayName == "":
		err = errors.New("missing email display name")
	case opt.PaginationHasher == nil:
		err = errors.New("missing pagination pagination hasher")
	...
	if opt.TemplatesDir != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to read template files in directory: %v", err)
		}
	...
	if !accountAPI.SQLDBWrites.Migrator().HasTable((&Account{}).TableName()) {
		err = accountAPI.SQLDBWrites.AutoMigrate(&Account{})
		if err != nil {
			return nil, fmt.Errorf("failed to automigrate %s table: %v", (&Account{}).TableName(), err)
		}
	}

	if !accountAPI.SQLDBWrites.Migrator().HasIndex(&Account{}, dbutil.FullTextIndex) {
		// Create a full text search index
		err = dbutil.CreateFullTextIndex(accountAPI.SQLDBWrites, accountsTable, "names", "email", "phone", "parent_id")
		if err != nil {
...
func (accountAPI *accountAPIServer) SignInExternal(
	...
	// Omit fields
	omitFields := []string{"project_id", "id_number", "linked_accounts", "password", "primary_group", "account_state", "parent_id", "secondary_groups", "security_question", "security-answer", "account_id", "gender", "created_at"}
...
func (accountAPI *accountAPIServer) RefreshSession(
	...
	case req.RefreshToken == "":
		return nil, errs.MissingField("refresh token")
	case req.AccountId == "":
...
func (accountAPI *accountAPIServer) ActivateAccount(
	...
	case activateReq == nil:
		return nil, errs.NilObject("activate accoun request")
