...
var (
	ChannelAPIServer *channelAPIServer
...
var _ = BeforeSuite(func() {
	...
	// Create channel API
	ChannelAPI, err = NewChannelAPIServer(ctx, opt)
	Expect(err).ShouldNot(HaveOccurred())
	...
	// 		Channel: mockChannel(),
	// 	}
	// 	createReq.Channel.Title = createReq.Channel.Title + " " + randomdata.RandStringRunes(32)
	// 	_, err = ChannelAPI.CreateChannel(ctx, createReq)
	// 	Expect(err).ShouldNot(HaveOccurred())
	// }
})
