...
var (
	EmailServer *emailingAPIServer
	EmailAPI    emailing.EmailingServer
	err         error
)
...
var _ = BeforeSuite(func() {
	...
	// Create email server
	EmailAPI, err = NewEmailingAPIServer(context.Background(), opt)
	Expect(err).ShouldNot(HaveOccurred())

	var ok bool
	EmailServer, ok = EmailAPI.(*emailingAPIServer)
	...
	opt.SMTPHost = randomdata.IpV4Address()
	opt.SMTPPort = 0
	_, err = NewEmailingAPIServer(ctx, opt)
	Expect(err).Should(HaveOccurred())
	...
	opt.SMTPUsername = randomdata.Email()
	opt.SMTPPassword = ""
	_, err = NewEmailingAPIServer(ctx, opt)
	Expect(err).Should(HaveOccurred())

	opt.SMTPPassword = randomdata.RandStringRunes(15)
	_, err = NewEmailingAPIServer(ctx, opt)
	Expect(err).ShouldNot(HaveOccurred())
})
