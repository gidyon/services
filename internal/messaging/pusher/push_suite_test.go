...
var (
	PushServer *pushAPIServer
	PushAPI    push.PushMessagingServer
	err        error
)
...
var _ = BeforeSuite(func() {
	...
	logger := micro.NewLogger("pusher", 0)
	...
	// Create push server
	PushAPI, err = NewPushMessagingServer(context.Background(), opt)
	Expect(err).ShouldNot(HaveOccurred())

	var ok bool
	PushServer, ok = PushAPI.(*pushAPIServer)
	...
	opt.FCMServerKey = randomdata.RandStringRunes(64)
	_, err = NewPushMessagingServer(ctx, opt)
	Expect(err).ShouldNot(HaveOccurred())
})
